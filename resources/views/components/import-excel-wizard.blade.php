...
<div class="p-8 max-w-5xl mx-auto">
    <a href="{{ $backRoute }}" wire:navigate class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 mb-6">
        <x-heroicon-o-arrow-left class="w-4 h-4" /> {{ $backLabel }}
    </a>
    <x-page-header :title="$title" :subtitle="$subtitle" icon="arrow-up-tray" />
    ...
    @if ($step === 'subir')
        <div class="bg-gradient-to-b from-white to-gray-50/60 dark:from-gray-900 dark:to-gray-900/70 rounded-xl border border-gray-200 dark:border-gray-800 shadow-md p-8 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Elegí un archivo Excel (.xlsx, .xls) o CSV con tus {{ $indexLabel !== '' ? Str::lower($entidadSingular).'s' : '' }}.</p>
    ...
    @if ($step === 'mapear')
        <div class="bg-gradient-to-b from-white to-gray-50/60 dark:from-gray-900 dark:to-gray-900/70 rounded-xl border border-gray-200 dark:border-gray-800 shadow-md p-5 mb-6">
            ...
            @foreach ($campos as $clave => $campo)
                @if ($campo['required'])
                    @error("mapeo.{$clave}") <p class="text-xs text-red-600 dark:text-red-400 mt-3">{{ $message }}</p> @enderror
                @endif
            @endforeach
    ...
    @if ($step === 'resultado' && $resultado)
        <div class="bg-gradient-to-b from-white to-gray-50/60 dark:from-gray-900 dark:to-gray-900/70 rounded-xl border border-gray-200 dark:border-gray-800 shadow-md p-8 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                {{ $resultado['creados'] }} {{ Str::plural($entidadSingular, $resultado['creados']) }} creado{{ $resultado['creados'] === 1 ? '' : 's' }},
            ...
            <div class="flex justify-center gap-3 mt-6">
                <a href="{{ $indexRoute }}" wire:navigate class="rounded-lg bg-gradient-to-r from-indigo-600 to-indigo-500 px-4 py-2 text-sm font-medium text-white shadow-md hover:from-indigo-700 hover:to-indigo-600">{{ $indexLabel }}</a>
